...
public class AutoPhrasingTokenFilter extends TokenFilter {
	...
	private static final ESLogger Log = ESLoggerFactory
			.getLogger(AutoPhrasingTokenFilter.class.getName());
	...
	public AutoPhrasingTokenFilter(TokenStream input,
			CharArraySet phraseSet, boolean emitSingleTokens) {
		super(input);
	...
	public boolean incrementToken() throws IOException {
		...
		if (nextToken == null) {
			...
			if (emitSingleTokens && currentSetToCheck != null
				...
				if (phrase != null && endsWith(lastTok, phrase)) {
					...
					&& currentSetToCheck.size() > 0) {
				if (lastEmitted != null
						&& !equals(fixWhitespace(lastEmitted),
								getCurrentBuffer(new char[0]))) {
		...
		if (currentSetToCheck == null || currentSetToCheck.size() == 0) {
			...
			if (phraseMap.keySet().contains(nextToken, 0, nextToken.length)) {
				...
				// currentSetTocheck
				currentSetToCheck = phraseMap.get(nextToken, 0,
						nextToken.length);
			...
			if (currentSetToCheck.contains(currentBuffer, 0,
			...
			if (phraseMap.keySet().contains(nextToken, 0, nextToken.length)) {
				...
				CharArraySet newSet = phraseMap.get(nextToken, 0,
						nextToken.length);
	...
	private boolean isPhrase(char[] phrase) {
		return phraseMap != null
				&& phraseMap.containsKey(phrase, 0, phrase.length);
	}
	...
	private CharArrayMap convertPhraseSet(CharArraySet phraseSet) {
		CharArrayMap<CharArraySet> phraseMap = new CharArrayMap(
				Version.LUCENE_46, 100, false);
		...
		while (phraseIt != null && phraseIt.hasNext()) {
			...
			CharArraySet itsPhrases = phraseMap.get(firstTerm, 0,
					firstTerm.length);
			if (itsPhrases == null) {
		...
		for (int i = 0; i < phrase.length(); i++) {
			...
			if (isSpaceChar(chAt) && i > lastSp) {
				...
				if (lastEmitted == null || !endsWith(lastEmitted, tok)) {
					...
					Log.debug("discard " + new String(tok) + ": "
